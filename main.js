...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (!port) {
      console.error(
        "[main] All fixed ports (3000-3002) are in use. Falling back to random port.",
      );
      try {
...
ipcMain.handle(
  "scan-folder",
  async (_event, folderPath, userTriggered = false) => {
    const win =
    ...
    try {
      ...
      if (userTriggered)
        win.webContents.send("scan-folder-status", { success: true });
      return { folderPath, folders, audioFiles, coverImage };
    } catch (err) {
      console.error("Error scanning folder:", err);
      if (userTriggered)
        win.webContents.send("scan-folder-status", { success: false });
      return null;
    }
  },
);
