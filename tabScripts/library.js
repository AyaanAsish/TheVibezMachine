...
window.electronAPI.onScanFolderStatus(({ success }) => {
      ...
      for (const folder of result.folders) {
        ...
        if (folderResult && folderResult.audioFiles.length > 0) {
          createPlaylistCard(
            libraryGrid,
            folder.name,
            folderResult.audioFiles,
            folderResult.coverImage,
          );
          hasPlaylists = true;
        }
      }
...
window.setLibraryPath = async (path) => {
  ...
  await window.electronAPI.scanFolder(path, true); // user triggered
  loadLibrary();
};
